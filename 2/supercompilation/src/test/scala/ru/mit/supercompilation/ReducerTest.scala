...
import ru.mit.supercompilation.parser.{ExprBuilder, ProgParser}
import ru.mit.supercompilation.printer.ExprPrinter
...
class ReducerTest extends FunSuite {
  ...
  test("Danya's test -- reduce + restore") {
    assertResult("(\\x0 . (\\x1 . (\\x2 . (\\x3 . ((x0 x2) ((x1 x2) x3))))))") {
      val ast = ProgParser("(\\ t1 . ((t1 (\\ n a1 x1 . n (\\ sva2 zva2 . ((a1 sva2) ((x1 sva2) zva2))))) (\\ a . a)) (\\ s z . z)) (\\ s1 z1 . s1 (s1 z1))")
      ExprPrinter(Reducer.reduce(ExprBuilder(ast), Nil))
    }
  }
}
